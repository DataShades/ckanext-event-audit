...
from ckanext.event_audit.worker import EventWriteThread


class FakeRepo:
    """Minimal stand-in for a repository.

    Used to inspect what the writer thread would have written without touching
    a real backend.
    """
...
class TestEnqueueEvent:
    def test_puts_event_into_the_queue(self, monkeypatch: pytest.MonkeyPatch):
        event_queue: queue.Queue[types.Event] = queue.Queue(maxsize=1)
        monkeypatch.setattr(worker, "event_queue", event_queue)

        event = _event()
        result = worker.enqueue_event(event)
    ...
    def test_drops_new_event_when_queue_is_full(self, monkeypatch: pytest.MonkeyPatch):
        ...
        monkeypatch.setattr(worker, "event_queue", full_queue)

        result = worker.enqueue_event(_event())

        assert result.status is False
        assert full_queue.qsize() == 1


class TestStartWriterThread:
    @pytest.fixture(autouse=True)
    def restore_worker_state(self, monkeypatch: pytest.MonkeyPatch):
        """Let the tests start their own thread without touching the real one."""
        monkeypatch.setattr(worker, "event_queue", worker.event_queue)
        monkeypatch.setattr(worker, "_write_thread", None)
        monkeypatch.setattr(worker.atexit, "register", lambda *args, **kwargs: None)
        monkeypatch.setattr(worker.EventWriteThread, "start", lambda self: None)

    @pytest.mark.ckan_config(config.CONF_QUEUE_SIZE, 7)
    def test_queue_is_bounded_by_config(self):
        worker.start_writer_thread()

        assert worker.event_queue.maxsize == 7

    def test_only_one_thread_is_started(self):
        worker.start_writer_thread()

        thread = worker._write_thread
        queue_ = worker.event_queue

        worker.start_writer_thread()

        assert thread is not None
        assert worker._write_thread is thread
        assert worker.event_queue is queue_
